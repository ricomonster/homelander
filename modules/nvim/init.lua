...
do
  ...
  local function map(lhs, wincmd, dir, desc)
    vim.keymap.set('n', lhs, function() nav(wincmd, dir) end, { silent = true, noremap = true, desc = desc })
  end
...
do
  ...
  vim.keymap.set('n', '\\', function()
    if MiniFiles.close() ~= nil then return end
    ...
    local root = vim.fs.root(file, { '.git', 'package.json' }) or vim.fn.getcwd()
    ...
    vim.schedule(function()
      ...
      while path ~= root and vim.startswith(path, root .. '/') do
        table.insert(branch, 1, path)
        path = vim.fs.dirname(path)
      end

      if path == root then
        table.insert(branch, 1, root)
  ...
  vim.pack.add {
    { src = gh 'oribarilan/lensline.nvim', version = 'v2.1.0' },
  }
...
-- mini.pick setup, keymaps, LSP picker mappings
-- ============================================================
do
  ...
  vim.keymap.set('n', '<leader>/', function() MiniExtra.pickers.buf_lines { scope = 'current' } end, { desc = 'Search current buffer' })

  vim.keymap.set('n', '<leader>sn', function() MiniPick.builtin.files { cwd = vim.fn.stdpath 'config' } end, { desc = 'Search Neovim config files' })

  vim.api.nvim_create_autocmd('LspAttach', {
    callback = function(event)
      local buf = event.buf

      local lsp_pick = function(keys, scope, desc)
        vim.keymap.set('n', keys, function() MiniExtra.pickers.lsp { scope = scope } end, { buffer = buf, desc = desc })
      end

      -- Find references for the word under your cursor.
      lsp_pick('grr', 'references', 'LSP references')

      -- Jump to the implementation of the word under your cursor.
      -- Useful when your language has ways of declaring types without an actual implementation.
      lsp_pick('gri', 'implementation', 'LSP implementation')

      -- Jump to the definition of the word under your cursor.
      -- This is where a variable was first declared, or where a function is defined, etc.
      -- To jump back, press <C-t>.
      lsp_pick('grd', 'definition', 'LSP definition')

      -- Fuzzy find all the symbols in your current document.
      -- Symbols are things like variables, functions, types, etc.
      lsp_pick('gO', 'document_symbol', 'LSP document symbols')

      -- Fuzzy find all the symbols in your current workspace.
      -- Similar to document symbols, except searches over your entire project.
      lsp_pick('gW', 'workspace_symbol', 'LSP workspace symbols')

      -- Jump to the type of the word under your cursor.
      -- Useful when you're not sure what type a variable is and you want to see
      -- the definition of its *type*, not where it was *defined*.
      lsp_pick('grt', 'type_definition', 'LSP type definition')
    end,
  })
end
...
do
  ...
  local function find_local_prettier(ctx)
    local modules = vim.fs.find('node_modules', {
      path = ctx.dirname,
      upward = true,
      type = 'directory',
      limit = math.huge,
    ...
    for _, node_modules in ipairs(modules) do
      local bin = vim.fs.joinpath(node_modules, '.bin', 'prettier')
      if vim.fn.executable(bin) == 1 then return bin, vim.fs.dirname(node_modules) end
    end
  end

  local function has_prettier_config(dir)
    if vim.fs.find(prettier_configs, {
      path = dir,
      upward = true,
      type = 'file',
      limit = 1,
    ...
    do
      local ok, data = pcall(function() return vim.json.decode(table.concat(vim.fn.readfile(package_json), '\n')) end)

      if ok and data and data.prettier then return true end
    end
    ...
    formatters = {
      prettier = {
        ...
        command = function(_, ctx)
          local bin = find_local_prettier(ctx)
          return bin or vim.fs.joinpath(ctx.dirname, 'node_modules', '.bin', 'prettier')
        end,
        cwd = function(_, ctx)
          local _, root = find_local_prettier(ctx)
          return root
        end,
        require_cwd = true,
        condition = function(_, ctx) return find_local_prettier(ctx) ~= nil and has_prettier_config(ctx.dirname) end,
      },
    },
  }
  ...
  local function eslint_fix_all(bufnr)
    ...
    if not response or response.err then return end

    for _, action in ipairs(response.result or {}) do
      if action.edit then vim.lsp.util.apply_workspace_edit(action.edit, client.offset_encoding) end

      if action.command then client:request_sync('workspace/executeCommand', action.command, 5000, bufnr) end
    end
  end

  vim.api.nvim_create_autocmd('BufWritePre', {
    group = vim.api.nvim_create_augroup('EslintFixAllOnSave', {}),
